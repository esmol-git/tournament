...
import type { AwardRow } from '~/types/admin/award'
...
const skeletonRows = Array.from({ length: SKELETON_ROW_COUNT }, (_, i) => ({ id: `aw-sk-${i}` }))
...
const { items, loading, error, isEmpty, run, retry } = useAdminAsyncListState<AwardRow>({
  initialLoading: true,
...
const editing = ref<AwardRow | null>(null)
...
const showNameError = computed(() => (submitAttempted.value || v$.value.name.$dirty) && !!formErrors.value.name)
...
const fetchItems = async () => {
  ...
  await run(async () => {
    items.value = await authFetch<AwardRow[]>(apiUrl(`/tenants/${tenantId.value}/awards`), {
...
const save = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await authFetch(apiUrl(`/tenants/${tenantId.value}/awards/${editing.value!.id}`), {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token.value}` },
        body,
      })
    } else {
      await authFetch(apiUrl(`/tenants/${tenantId.value}/awards`), {
        method: 'POST',
        headers: { Authorization: `Bearer ${token.value}` },
        body,
      })
...
const deleteTarget = ref<AwardRow | null>(null)
const deleteSaving = ref(false)

const openDeleteDialog = (row: AwardRow) => {
...
const confirmDelete = async () => {
  ...
  try {
    await authFetch(apiUrl(`/tenants/${tenantId.value}/awards/${deleteTarget.value.id}`), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token.value}` },
    })
...
<template>
  <section class="admin-page">
    <header class="admin-toolbar-responsive flex flex-wrap items-center justify-between gap-2 sm:gap-3">
      <div>
        <h1 class="text-lg font-semibold text-surface-900 dark:text-surface-0 sm:text-xl">Награды</h1>
        <p class="mt-1 max-w-2xl text-xs leading-relaxed text-muted-color sm:text-sm">
          Номинации и типы призов для турниров: MVP, лучший игрок финала, лучший бомбардир и другие.
        </p>
      </div>
    ...
    <AdminDataState
      ...
      :error="error"
      :empty="isEmpty"
      empty-title="Пока нет наград"
      empty-description="Добавьте номинации, чтобы использовать единый справочник для турнирных награждений."
      empty-icon="pi pi-star"
      error-title="Не удалось загрузить награды"
      @retry="retry"
    >
      <template #loading>
        <div class="min-h-[22rem]">
          <div
            class="grid grid-cols-1 sm:grid-cols-[minmax(0,2fr)_minmax(0,5rem)_minmax(0,4rem)_minmax(0,5rem)_minmax(0,1.5fr)_auto] gap-3 px-4 py-3 border-b border-surface-200 dark:border-surface-700 bg-surface-50/80 dark:bg-surface-800/50"
          >
          ...
          <div
            ...
            :key="row.id"
            class="grid grid-cols-1 sm:grid-cols-[minmax(0,2fr)_minmax(0,5rem)_minmax(0,4rem)_minmax(0,5rem)_minmax(0,1.5fr)_auto] gap-3 items-center px-4 py-3.5 border-b border-surface-100 dark:border-surface-800 last:border-0"
          >
            ...
            <Skeleton width="65%" height="0.875rem" class="rounded-md max-w-md" />
    ...
    >
      <div class="flex flex-col gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <label for="award_active" class="text-sm cursor-pointer">Активна</label>
        </div>
        <div>
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        Будет удалена награда <span class="font-semibold text-surface-900 dark:text-surface-0">{{ deleteTarget.name }}</span>.
      </p>
      <template #footer>
        <div class="flex flex-wrap justify-end gap-2">
          <Button type="button" label="Отмена" text :disabled="deleteSaving" @click="deleteDialogVisible = false" />
          <Button type="button" label="Удалить" icon="pi pi-trash" severity="danger" :loading="deleteSaving" @click="confirmDelete" />
        </div>
      </template>
    </Dialog>
  </section>
