...
const { items, loading, error, isEmpty, run, retry } = useAdminAsyncListState<RegionRow>({
  initialLoading: true,
...
const deleteTarget = ref<RegionRow | null>(null)
const deleteSaving = ref(false)

const openDeleteDialog = (row: RegionRow) => {
...
<template>
  <section class="admin-page">
    <header class="admin-toolbar-responsive flex flex-wrap items-center justify-between gap-2 sm:gap-3">
      <div>
        <h1 class="text-lg font-semibold text-surface-900 dark:text-surface-0 sm:text-xl">Регионы</h1>
    ...
    <AdminDataState
      ...
      empty-description="Добавьте регионы и выберите их у команд или стадионов в соответствующих разделах."
      empty-icon="pi pi-map-marker"
      error-title="Не удалось загрузить регионы"
      @retry="retry"
    >
      <template #loading>
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        ...
        будет удалена. Если регион уже указан у команд или стадионов, операция может быть отклонена
        сервером.
      </p>
      <template #footer>
