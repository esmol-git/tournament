...
const items = ref<RefereeCategoryRow[]>([])
...
const fetchItems = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Не удалось загрузить категории',
      life: 5000,
    })
  } finally {
...
const remove = async (row: RefereeCategoryRow) => {
  if (!token.value) return
  if (!confirm(`Удалить категорию «${row.name}»? Судьи останутся без этой категории.`)) return
  try {
    await authFetch(
      apiUrl(`/tenants/${tenantId.value}/referee-categories/${row.id}`),
      {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token.value}` },
      },
...
<template>
  <section class="p-6 space-y-4">
    ...
    <Transition name="rcat-fade" mode="out-in">
      <div
        v-if="loading"
        key="sk"
      ...
      >
        ...
        >
          <i class="pi pi-bookmark text-3xl" aria-hidden="true" />
        </div>
        ...
        <p class="mt-2 text-sm text-muted-color max-w-md mx-auto leading-relaxed">
          Добавьте категории (например, 1‑я, 2‑я, всероссийская), затем назначайте их судьям в справочнике.
        </p>
        <Button
          class="mt-6"
...
<style scoped>
.rcat-fade-enter-active,
.rcat-fade-leave-active {
  transition: opacity 0.22s ease;
}
.rcat-fade-enter-from,
.rcat-fade-leave-to {
  opacity: 0;
}
</style>
