...
const { items, loading, error, isEmpty, run, retry } = useAdminAsyncListState<RefereeCategoryRow>({
  initialLoading: true,
...
const editing = ref<RefereeCategoryRow | null>(null)
...
const fetchItems = async () => {
  ...
  await run(async () => {
    items.value = await authFetch<RefereeCategoryRow[]>(
...
const openEdit = (row: RefereeCategoryRow) => {
  submitAttempted.value = false
  editing.value = row
  form.name = row.name
  form.code = row.code ?? ''
...
const save = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await authFetch(
        apiUrl(`/tenants/${tenantId.value}/referee-categories/${editing.value!.id}`),
        {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${token.value}` },
          body,
      ...
      await authFetch(apiUrl(`/tenants/${tenantId.value}/referee-categories`), {
        method: 'POST',
        headers: { Authorization: `Bearer ${token.value}` },
        body,
      })
...
const deleteTarget = ref<RefereeCategoryRow | null>(null)
const deleteSaving = ref(false)

const openDeleteDialog = (row: RefereeCategoryRow) => {
...
const confirmDelete = async () => {
  ...
  try {
    await authFetch(apiUrl(`/tenants/${tenantId.value}/referee-categories/${row.id}`), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token.value}` },
    })
...
<template>
  <section class="admin-page">
    <header class="admin-toolbar-responsive flex flex-wrap items-center justify-between gap-2 sm:gap-3">
      <div>
        ...
        <p class="mt-1 max-w-2xl text-xs leading-relaxed text-muted-color sm:text-sm">
          Разряды и квалификация (всероссийская, региональная и т.д.). Значения выбираются в карточке
          судьи и помогают единообразно описывать состав судейской коллегии.
        </p>
      </div>
    ...
    <AdminDataState
      ...
      empty-description="Добавьте категории (например, 1‑я, 2‑я, всероссийская), затем назначайте их судьям в справочнике."
      empty-icon="pi pi-bookmark"
      error-title="Не удалось загрузить категории"
      @retry="retry"
    >
      <template #loading>
    ...
    <Dialog
      ...
      modal
      block-scroll
      :header="isEdit ? 'Редактировать категорию' : 'Новая категория'"
      :style="{ width: '26rem' }"
    >
      <div class="flex flex-col gap-3">
        <div>
          <label class="text-sm block mb-1">Название *</label>
          <InputText v-model="form.name" class="w-full" :invalid="showNameError" placeholder="Например, 1-я категория" />
        ...
        <div>
          ...
          <div class="flex gap-2">
            <InputText v-model="form.code" class="w-full" placeholder="CAT1, RFL…" @input="codeTouched = true" />
        ...
        <div class="flex items-center gap-2">
          <ToggleSwitch v-model="form.active" input-id="rcat_active" />
          <label for="rcat_active" class="text-sm cursor-pointer">Активна</label>
        </div>
        <div>
    ...
    <Dialog
      ...
      modal
      block-scroll
      header="Удалить категорию судей?"
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        ...
        будет удалена. У судей, у которых она была выбрана, поле категории очистится.
      </p>
      <template #footer>
