...
<template>
  <section class="admin-page">
    <header class="admin-toolbar-responsive flex flex-wrap items-center justify-between gap-2 sm:gap-3">
      <div>
        <h1 class="text-lg font-semibold text-surface-900 dark:text-surface-0 sm:text-xl">Возрастные группы</h1>
        <p class="mt-1 max-w-2xl text-sm leading-relaxed text-muted-color">
          <span class="text-surface-700 dark:text-surface-200">Это только подпись-дивизион</span> (например,
          U-17 или взрослые): её видно в шапке турнира, в фильтрах списка и на сайте, её можно указать у
          команды. Здесь <strong class="font-medium text-surface-800 dark:text-surface-100">нет</strong>
          полей «год рождения с… по…» — платформа <strong class="font-medium text-surface-800 dark:text-surface-100">не проверяет</strong>,
          что игроки реально подходят под букву U-17.
          <span class="mt-1 block">
            Чтобы не пускать в состав «не тех» по возрасту или полу, настройте
            <strong class="font-medium text-surface-800 dark:text-surface-100">«Категории команд»</strong>
            и привяжите категорию к команде — там как раз задаются годы рождения и ограничения по составу.
          </span>
          <span class="mt-1 block">
            Если у турнира выбрана возрастная группа, проверяется лишь согласованность: команды в турнире
            должны быть с той же возрастной группой, что и турнир.
          </span>
        </p>
      </div>
    ...
    <AdminDataState
      ...
      :error="error"
      :empty="isEmpty"
      empty-title="Пока нет записей"
      empty-description="Создайте метки (U-17, взрослые…) и укажите их у команды или турнира. Жёсткий контроль возраста игроков в составе — в справочнике «Категории команд»."
      empty-icon="pi pi-chart-bar"
      error-title="Не удалось загрузить возрастные группы"
      @retry="retry"
    >
      <template #loading>
        <div class="min-h-[22rem]">
          <div
            class="grid grid-cols-1 sm:grid-cols-[minmax(0,1.6fr)_minmax(0,4rem)_minmax(0,4rem)_minmax(0,4rem)_minmax(0,4rem)_auto] gap-3 px-4 py-3 border-b border-surface-200 dark:border-surface-700 bg-surface-50/80 dark:bg-surface-800/50"
          >
          ...
          <div
            ...
            :key="row.id"
            class="grid grid-cols-1 sm:grid-cols-[minmax(0,1.6fr)_minmax(0,4rem)_minmax(0,4rem)_minmax(0,4rem)_minmax(0,4rem)_auto] gap-3 items-center px-4 py-3.5 border-b border-surface-100 dark:border-surface-800 last:border-0"
          >
      ...
      <DataTable :value="items" data-key="id" striped-rows>
        ...
        <Column field="shortLabel" header="Коротко" style="width: 5rem" />
    ...
    <Dialog
      ...
      modal
      block-scroll
      :header="isEdit ? 'Редактировать группу' : 'Новая возрастная группа'"
      :style="{ width: 'min(26rem, 100vw - 2rem)' }"
    >
      <div class="flex flex-col gap-3">
        <div>
          <label class="text-sm block mb-1">Название *</label>
          <InputText v-model="form.name" class="w-full" :invalid="showNameError" placeholder="Например, Дети до 12 лет" />
          <p v-if="showNameError" class="mt-0 text-[11px] leading-3 text-red-500">{{ formErrors.name }}</p>
        </div>
        <div>
          <label class="text-sm block mb-1">Короткая подпись</label>
          <InputText v-model="form.shortLabel" class="w-full" placeholder="U12" />
        </div>
        <div>
        ...
        <div class="flex items-center gap-2">
          ...
          <label for="ag_active" class="text-sm cursor-pointer">Активна</label>
        </div>
        <div>
    ...
    <Dialog
      ...
      modal
      block-scroll
      header="Удалить возрастную группу?"
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        ...
        будет удалена. Если группа уже указана в турнирах или командах, операция может быть отклонена
        сервером.
      </p>
      <template #footer>
