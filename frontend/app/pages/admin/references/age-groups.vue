...
const fetchItems = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Не удалось загрузить возрастные группы',
      life: 5000,
    })
  } finally {
...
const remove = async (row: AgeGroupRow) => {
  if (!token.value) return
...
<template>
  <section class="p-6 space-y-4">
    <header class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-semibold text-surface-900 dark:text-surface-0">Возрастные группы</h1>
        <p class="mt-1 text-sm text-muted-color max-w-2xl">
          Подписи и дивизионы для отображения и фильтров (U12, взрослые, любители): привязка к команде и
          турниру, каталог на сайте.
        </p>
      </div>
    ...
    <Transition name="ag-fade" mode="out-in">
      ...
      >
        ...
        >
          <i class="pi pi-chart-bar text-3xl" aria-hidden="true" />
        </div>
        ...
        <p class="mt-2 text-sm text-muted-color max-w-md mx-auto leading-relaxed">
          Создайте группы и укажите их у команд или в карточке турнира — список можно фильтровать по
          возрасту.
        </p>
