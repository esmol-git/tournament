...
const { items, loading, error, isEmpty, run, retry } = useAdminAsyncListState<MatchScheduleReasonRow>({
  initialLoading: true,
...
const deleteTarget = ref<MatchScheduleReasonRow | null>(null)
const deleteSaving = ref(false)

const openDeleteDialog = (row: MatchScheduleReasonRow) => {
...
<template>
  <section class="admin-page">
    ...
    <AdminDataState
      ...
      empty-description="Создайте причины для переносов и отмен матчей."
      empty-icon="pi pi-calendar-times"
      error-title="Не удалось загрузить причины"
      @retry="retry"
    >
      <template #loading>
    ...
    <Dialog
      ...
      modal
      block-scroll
      header="Удалить причину переноса / отмены?"
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        ...
        будет удалена. Если причина уже указана в журнале матчей, операция может быть отклонена сервером.
      </p>
      <template #footer>
