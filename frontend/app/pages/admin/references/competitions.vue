...
const items = ref<CompetitionRow[]>([])
...
const fetchItems = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Не удалось загрузить соревнования', life: 5000 })
  } finally {
...
const remove = async (row: CompetitionRow) => {
  if (!token.value) return
...
<template>
  <section class="p-6 space-y-4">
    <header class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-semibold text-surface-900 dark:text-surface-0">Соревнования</h1>
    ...
    <Transition name="cmp-fade" mode="out-in">
      <div
        v-if="loading"
        key="sk"
      ...
      >
        ...
        >
          <i class="pi pi-flag text-3xl" aria-hidden="true" />
        </div>
        ...
        <p class="mt-2 text-sm text-muted-color max-w-md mx-auto leading-relaxed">
          Добавьте записи и привяжите тип к турниру в разделе «Турниры» или отфильтруйте список по типу.
        </p>
