...
const skeletonRows = Array.from({ length: SKELETON_ROW_COUNT }, (_, i) => ({ id: `cmp-sk-${i}` }))
...
const { items, loading, error, isEmpty, run, retry } = useAdminAsyncListState<CompetitionRow>({
  initialLoading: true,
...
const editing = ref<CompetitionRow | null>(null)
...
const fetchItems = async () => {
  ...
  await run(async () => {
    items.value = await authFetch<CompetitionRow[]>(
      apiUrl(`/tenants/${tenantId.value}/competitions`),
...
const openEdit = (row: CompetitionRow) => {
  submitAttempted.value = false
  editing.value = row
  form.name = row.name
  form.code = row.code ?? ''
...
const save = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await authFetch(apiUrl(`/tenants/${tenantId.value}/competitions/${editing.value!.id}`), {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token.value}` },
        body,
      })
    } else {
      await authFetch(apiUrl(`/tenants/${tenantId.value}/competitions`), {
        method: 'POST',
        headers: { Authorization: `Bearer ${token.value}` },
        body,
      })
...
const openDeleteDialog = (row: CompetitionRow) => {
...
const confirmDelete = async () => {
  ...
  try {
    await authFetch(apiUrl(`/tenants/${tenantId.value}/competitions/${row.id}`), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token.value}` },
    })
...
<template>
  <section class="admin-page">
    <header class="admin-toolbar-responsive flex flex-wrap items-center justify-between gap-2 sm:gap-3">
      <div>
        <h1 class="text-lg font-semibold text-surface-900 dark:text-surface-0 sm:text-xl">Соревнования</h1>
        <p class="mt-1 max-w-2xl text-xs leading-relaxed text-muted-color sm:text-sm">
          Здесь задаются виды соревнований тенанта: чемпионат, кубок, первенство и т.д. В карточке
          турнира («Турниры») можно выбрать тип — он отображается в шапке турнира, участвует в
          фильтрах списка и доступен на публичном сайте вместе с сезоном.
        </p>
      </div>
    ...
    <AdminDataState
      ...
      empty-description="Добавьте записи и привяжите тип к турниру в разделе «Турниры» или отфильтруйте список по типу."
      empty-icon="pi pi-flag"
      error-title="Не удалось загрузить соревнования"
      @retry="retry"
    >
      <template #loading>
    ...
    <Dialog
      ...
      modal
      block-scroll
      :header="isEdit ? 'Редактировать' : 'Новое соревнование'"
      :style="{ width: 'min(26rem, 100vw - 2rem)' }"
    >
      <div class="flex flex-col gap-3">
        <div>
          <label class="text-sm block mb-1">Название *</label>
          <InputText v-model="form.name" class="w-full" :invalid="showNameError" placeholder="Например, Чемпионат города" />
        ...
        <div class="flex items-center gap-2">
          <ToggleSwitch v-model="form.active" input-id="cmp_active" />
          <label for="cmp_active" class="text-sm cursor-pointer">Активна</label>
        </div>
        <div>
    ...
    <Dialog
      ...
      modal
      block-scroll
      header="Удалить тип соревнования?"
    ...
    >
      <p v-if="deleteTarget" class="text-sm text-surface-700 dark:text-surface-200">
        ...
        будет удалена. Если тип уже указан в турнирах, операция может быть отклонена сервером.
      </p>
      <template #footer>
