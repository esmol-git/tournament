...
type SlotLabels = { home: string; away: string }
...
function matchHasResult(m: MatchRow): boolean {
  return (
    m.homeScore !== null &&
    m.awayScore !== null &&
...
function resolvedWinnerSide(m: MatchRow): 'HOME' | 'AWAY' | null {
  ...
  // Playoff ties can be resolved by penalties in protocol meta.
  const pen = readPenaltyScore(m)
  if (!pen || pen.home === pen.away) return null
  return pen.home > pen.away ? 'HOME' : 'AWAY'
}

function winnerName(m: MatchRow): string | null {
...
function loserName(m: MatchRow): string | null {
  const side = resolvedWinnerSide(m)
...
function buildMatchNumberById(matches: MatchRow[]): Record<string, number> {
...
function groupStageIsFinished(matches: MatchRow[]): boolean {
...
export function buildPlayoffSlotLabels(
  ...
  // Build seed labels directly from actual first-round bracket order.
