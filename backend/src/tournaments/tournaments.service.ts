...
export class TournamentsService {
  ...
  async resolveTenantForPublicOrThrow(slug: string) {
    const tenant = await this.prisma.tenant.findUnique({
      where: { slug },
      select: { id: true, name: true, slug: true, blocked: true },
    });
  ...
  async update(id: string, dto: UpdateTournamentDto) {
    ...
    if (dto.category !== undefined) {
      const badTeam = await this.prisma.tournamentTeam.findFirst({
        ...
        select: { team: { select: { name: true, category: true } } },
      });
      if (badTeam) {
        throw new BadRequestException(
          `Cannot set tournament category "${dto.category}" because team "${badTeam.team.name}" has category "${badTeam.team.category ?? '—'}"`,
        );
      }
    }
  ...
  async addTeam(tournamentId: string, teamId: string) {
    const tournament = await this.prisma.tournament.findUnique({
      where: { id: tournamentId },
      select: { id: true, tenantId: true, status: true, category: true },
    });
    ...
    if (hasEnteredResults) {
      throw new BadRequestException(
        'Can add teams only before entering results',
      );
    }

    const team = await this.prisma.team.findUnique({
      where: { id: teamId },
      select: { id: true, tenantId: true, rating: true, category: true },
    });
    if (!team) throw new BadRequestException('Team not found');
    if (team.tenantId !== tournament.tenantId) {
      throw new BadRequestException('Team must belong to same tenant');
    }
    if (tournament.category && team.category !== tournament.category) {
      throw new BadRequestException(
        `Team category "${team.category ?? '—'}" does not match tournament category "${tournament.category}"`,
      );
    }
  ...
  async removeTeam(tournamentId: string, teamId: string) {
    ...
    if (hasEnteredResults) {
      throw new BadRequestException(
        'Can remove teams only before entering results',
      );
    }
  ...
  async setTeamGroup(
    ...
    if (hasEnteredResults) {
      throw new BadRequestException(
        'Cannot change groups after entering results',
      );
    }
  ...
  async generateCalendar(tournamentId: string, dto: GenerateCalendarDto) {
    ...
    if (tournament.format === TournamentFormat.SINGLE_GROUP) {
      ...
      const rr = buildRoundRobinRounds(orderedTeams);
    ...
    const scheduleRoundWithSlots = (
      ...
      roundStart.setHours(hh, mm, 0, 0);
      const roundStartMs = roundStart.getTime() + dayOffsetMinutes * 60 * 1000;

      let matchIndex = 0;
      for (const m of planned.matches) {
        const wave = Math.floor(matchIndex / simultaneousMatches);
        const offsetMinutes = wave * (matchDurationMinutes + matchBreakMinutes);
        const startTime = new Date(roundStartMs + offsetMinutes * 60 * 1000);
        matchesToCreate.push({
          tenantId: tournament.tenantId,
    ...
    for (let r = 0; r < roundsToSchedule.length; r++) {
      const roundIndexInDay = r % roundsPerDay;
      if (r > 0 && roundIndexInDay === 0) {
        roundDate = new Date(
      ...
      // When multiple rounds are placed on the same day, shift start time forward to avoid overlaps.
      let dayOffsetMinutes = 0;
      if (roundIndexInDay > 0) {
        for (let j = r - roundIndexInDay; j < r; j++) {
          const prev = roundsToSchedule[j];
          const waves = Math.ceil(
            (prev.matches?.length ?? 0) / simultaneousMatches,
          );
          dayOffsetMinutes +=
            waves * (matchDurationMinutes + matchBreakMinutes);
        }
      }
      scheduleRoundWithSlots(roundDate, roundsToSchedule[r], dayOffsetMinutes);
    }
